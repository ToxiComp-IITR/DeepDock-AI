...
const ProteinViewer3D: React.FC<ProteinViewer3DProps> = ({ 
  ...
  const loadPDBStructure = async (viewer: any, pdbIdToLoad: string): Promise<boolean> => {
    try {
      ...
      // Import PDB utilities
      const { fetchPDBStructure } = await import('@/utils/pdbUtils');
      const pdbData = await fetchPDBStructure(pdbIdToLoad);
  ...
  const fetchProteinInfo = async (pdbIdToFetch: string) => {
    try {
      const { fetchPDBInfo } = await import('@/utils/pdbUtils');
      const info = await fetchPDBInfo(pdbIdToFetch);
      ...
      if (info) {
        setProteinInfo({
          ...
          depositionDate: info.depositionDate || 'Unknown'
        });
      }
    } catch (e) {
