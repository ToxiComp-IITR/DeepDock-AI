...
const MoleculeViewer2D: React.FC<MoleculeViewer2DProps> = ({ 
  ...
  const tryPubChemStructure = async (): Promise<boolean> => {
    try {
      ...
      const { fetchPubChemCID, getPubChem2DImageURL } = await import('@/utils/pubchemUtils');
      ...
      const imageUrl = getPubChem2DImageURL(cid, width, height);
      
      return await loadImageToCanvas(imageUrl);
    } catch (e) {
      console.error('PubChem structure fetch error:', e);
      return false;
    }
  };
