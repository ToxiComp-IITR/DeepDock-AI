    ...
    try {
      ...
      if (viewerRef.current) {
        ...
        // Try to fetch from PubChem if we have an ID
          ...
          try {
            const { fetchPubChem3DStructure } = await import('@/utils/pubchemUtils');
            const sdfData = await fetchPubChem3DStructure(pubchemId);
            if (sdfData) {
        ...
        // Try PubChem 3D from SMILES if no other source worked
        if (!structureLoaded && smiles) {
          try {
            const { fetchPubChemCID, fetchPubChem3DStructure } = await import('@/utils/pubchemUtils');
            const cid = await fetchPubChemCID(smiles);
            if (cid) {
              const sdfData = await fetchPubChem3DStructure(cid);
              if (sdfData) {
                newViewer.addModel(sdfData, 'sdf');
                structureLoaded = true;
              }
            ...
            console.log('PubChem fetch from SMILES failed');
          }
          
          // Generate fallback structure if PubChem fails
          if (!structureLoaded) {
