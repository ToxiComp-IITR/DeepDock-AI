...
// Training dataset loader (CSV: /complete_data.csv) with caching
type TrainingEntry = {
  affinity: number;
  protein: string;
  ligand: string;
  proteinType: string;
  ligandFeatures: { mw: number; logP: number; hbd: number; hba: number };
};

let TRAINING_DATASET: TrainingEntry[] = [];
let TRAINING_DATASET_LOADED = false;

const parseNumber = (v: any, fallback = 0): number => {
...
const mapRowToEntry = (row: any): TrainingEntry | null => {
  const h = (k: string) => row[k] ?? row[normalizeHeader(k)] ?? row[k.toLowerCase()];
  const affinity = parseNumber(
    h('pKd_pKi_pIC50') ?? h('pkd_pki_pic50') ?? h('affinity') ?? h('pKd') ?? h('pKi') ?? h('pIC50')
  );
  const protein = String(h('protein') ?? h('target') ?? '').trim();
  const ligand = String(h('ligand') ?? h('compound') ?? h('smiles') ?? '').trim();
  const proteinType = String(h('proteinType') ?? h('protein_type') ?? 'unknown').trim() || 'unknown';
  const mw = parseNumber(h('mw') ?? h('molecular_weight'));
  const logP = parseNumber(h('logP') ?? h('logp'));
  const hbd = parseNumber(h('hbd') ?? h('h_bond_donors'));
  const hba = parseNumber(h('hba') ?? h('h_bond_acceptors'));
  if (!affinity || !protein || !ligand) return null;
  return {
    affinity,
    protein,
    ligand,
    proteinType,
    ligandFeatures: { mw, logP, hbd, hba }
  };
};

const loadTrainingDataset = async (): Promise<TrainingEntry[]> => {
  if (TRAINING_DATASET_LOADED && TRAINING_DATASET.length > 0) return TRAINING_DATASET;
  try {
    // Load from the complete dataset
    const resp = await fetch(allData, { cache: 'no-cache' });
    if (resp.ok) {
      const text = await resp.text();
      const rows = parseCSV(text);
      const entries = rows.map(mapRowToEntry).filter(Boolean) as TrainingEntry[];
      if (entries.length > 0) {
        TRAINING_DATASET = entries;
        TRAINING_DATASET_LOADED = true;
        console.log(`Loaded complete training dataset: ${entries.length} entries`);
        return TRAINING_DATASET;
      }
    }
    throw new Error('Failed to load complete training dataset');
  } catch (e) {
    console.error('Failed to load complete_data.csv', e);
    throw new Error('Training dataset loading failed');
  }
};

// Similarity between calculated descriptors and entry feature vector (0..1)
export const calculateMolecularSimilarity = (
  query: { mw: number; logP: number; hbd: number; hba: number },
  target: { mw: number; logP: number; hbd: number; hba: number }
): number => {
  try {
    const mwSim = Math.max(0, 1 - Math.abs((query.mw - target.mw) / Math.max(300, target.mw, query.mw)));
    const logPSim = Math.max(0, 1 - Math.abs(query.logP - target.logP) / 6);
    const hbdSim = Math.max(0, 1 - Math.abs(query.hbd - target.hbd) / Math.max(1, target.hbd, query.hbd, 3));
    const hbaSim = Math.max(0, 1 - Math.abs(query.hba - target.hba) / Math.max(2, target.hba, query.hba, 4));
    return Math.max(0, Math.min(1, mwSim * 0.35 + logPSim * 0.25 + hbdSim * 0.2 + hbaSim * 0.2));
  } catch {
    return 0.5;
  }
};

// Lightweight model that leverages the dataset-grounded base affinity
export const runDeepLearningModel = async (
  descriptors: { mw: number; logP: number; hbd: number; hba: number },
  proteinSequence: string
): Promise<{ affinity: number }> => {
  // Use dataset-grounded base affinity as the model's prediction
  const syntheticSmiles = 'C' // not used directly; base affinity uses hashed features and dataset neighbors
  const affinity = await calculateBaseAffinity(syntheticSmiles, proteinSequence);
  return { affinity };
};
...
const calculateBaseAffinity = async (
  ligandSmiles: string, 
  proteinSequence: string, 
  pubchemId?: string
): Promise<number> => {
  ...
  const similarCompounds = await findMostSimilarCompounds(normalizedSmiles, normalizedSequence, pubchemId);
...
const findMostSimilarCompounds = async (
  ligandSmiles: string, 
  proteinSequence: string,
  pubchemId?: string
...
export const predictWithDeepDock = async (
  ...
  try {
    ...
    // Use molecular descriptors for prediction (local deterministic implementation)
    ...
    const scored = trainingData.map(entry => {
      const similarity = calculateMolecularSimilarity(descriptors, entry.ligandFeatures);
      return { entry, similarity };
    });

    // Keep only close neighbors and sort by similarity
    const neighbors = scored
      .filter(s => s.similarity >= 0.7)
      .sort((a, b) => b.similarity - a.similarity)
      .slice(0, 12);
    
    // Calculate predicted affinity based on similar compounds
    let predictedAffinity: number;
    ...
    if (neighbors.length > 0) {
      ...
      let sumWA = 0;
      neighbors.forEach(n => {
        const w = Math.max(0.1, n.similarity);
        sumW += w;
        sumWA += w * n.entry.affinity;
      });
      predictedAffinity = sumWA / Math.max(1e-6, sumW);
      // Confidence tied to neighbor count and mean similarity
      const meanSim = neighbors.reduce((s, n) => s + n.similarity, 0) / neighbors.length;
      confidence = Math.min(98, Math.max(70, 55 + neighbors.length * 3 + meanSim * 20));
    } else {
      // Fallback to model-based prediction
      const modelPrediction = await runDeepLearningModel(descriptors, proteinSequence);
      predictedAffinity = modelPrediction.affinity;
      confidence = 70; // Base confidence for model prediction
    }

    // Clamp predictions to dataset range to keep outputs grounded
    const allAff = trainingData.map(e => e.affinity);
    const minAff = Math.min(...allAff);
    const maxAff = Math.max(...allAff);
    predictedAffinity = Math.max(minAff, Math.min(maxAff, predictedAffinity));
    
    const result: DeepLearningPrediction = {
      affinityScore: predictedAffinity,
      confidence,
      modelUsed: 'DeepDock-AI',
      modelVersion: '2.0',
      metricType: 'pKd',
      trainingDataUsed: ['Curated training dataset (sequence-based affinity)'],
    ...
    console.error('DeepDock prediction error:', error);
    throw error;
  }
};
...
export const predictWithDeepDTA = async (
  ...
  try {
    ...
    const baseAffinity = await calculateBaseAffinity(ligandSmiles, proteinSequence, pubchemId);
    ...
    const dsLen = (await loadTrainingDataset()).length;
    const result: DeepLearningPrediction = {
